/**
 * Created by Мирослав on 25.04.2016.
 */
var jsonData = [
    {
...
(function() {
var skillsArray = _.map(dataString, 'skills'); //Вытягиваем все массивы skills
var allSkills = _.union(_.flatMap(skillsArray)); //Убираем все дубликаты значений skills
var sortedSkills = _.sortBy(allSkills);
...
(function() {
var nameArray = _.flatMap(dataString, 'name'); //Массив имён
var friendsArray = _.map(dataString, 'friends'); //Массив друзей
// var mapValues = _.mapValues(dataString, 'friends');
var friendsCount = [], j=0;
_(friendsArray).forEach(function(val) { //Создаём массив колличества друзей
    friendsCount[j] = {'name': nameArray[j], 'friends' : _.size(val)};
    j++;
});
// var zipArrays = _.zipObject(nameArray, friendsCount); //Обединяем массивы вобъект в формате { имя: колличество, имя: колличество } для каждого имени
var sortedByFriends = _.orderBy(friendsCount, ['friends', 'name'], ['desc', 'asc']); //Сортировка
var a = 0;
console.log ('\n2. Массив имен (поле name) людей, отсортированных в зависимости от количества их друзей (friends):\n');
_(sortedByFriends).forEach(function(val) {
    console.log(sortedByFriends[a].name, ' :', sortedByFriends[a].friends);
    a++;
});
}());
...
(function() {
    ...
    var allFriendsArray = _.flatMapDeep(dataString, 'friends'); //Массив всех друзей
    var i = 0;
    var friendsArray= [];
    _(allFriendsArray).forEach(function(val) { //Вытягиваем все имена
        friendsArray[i++] = val.name;
    });
    var unuqFriends = _.union(friendsArray); //Убираем повторяющихся друзей
    console.log('\n3. Массив всех друзей всех пользователей, не должно быть повторяющихся людей:\n');
    console.log(unuqFriends);
}());